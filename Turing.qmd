...
Pkg.instantiate()
```
`Turing` is a PPL package that
-   uses syntax similar to model mathematical specification

-   enables various sampling methods as well as diagnostics through its package ecosystem

Let's start by creating a model using `Turing`. To do this, we use the `@model` macro. In this macro,

-   Specification of distribution statement uses the `~` notation, e.g.

    -   $x\sim N(0,1)$ is `x~Normal(0,1)`

    -   $v_i \sim \text{Cauchy}(2,1)$ is `v[i]~Cauchy(2,1)`

-   Distribution syntax in `Turing` follows `Distribution`. See [Distribution docs](https://juliastats.org/Distributions.jl/stable/) for a list of common distribution families.

-   Other relations follows regular julia code (transformations, assignments, indexing, etc)
...
@model function gaussian_mix(x)
    ...
    n, d = size(x) # sample size and dimension of data
    distribution_clusters = [MvNormal(Fill(μₖ, d), I) for μₖ in μ]

    # Draw assignments for each datum and generate it from the multivariate normal distribution.
    k = Vector{Int}(undef, n)
    for i in 1:n
        z[i] ~ distribution_assignments
        x[:, i] ~ distribution_clusters[z[i]]
    end
...
```{julia}
# log posterior
@show logjoint(model, (a = a,));

# log prior
@show logprior(model, (a = a,));

# log likelihood
@show loglikelihood(model, (a = a,));
```
...
For more functionalities and uses, see [Turing docs](https://turinglang.org/docs/getting-started/)