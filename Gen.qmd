...
`Gen` is another PPL package that came out around the same time as `Turing`. For `Gen`, the model creation process is similar to `Turing` except for a few differences
...
For more functionalities and uses, see [Gen docs](https://www.gen.dev/docs/stable/)